...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        shortcut_type='B', num_classes=2,dropout=0.5,ST_struc=('A','B','C')):
        self.inplanes = 64
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    if pretrained==True:
        if modality=='RGB':
            pretrained_file='./weightFile/p3d_rgb_199.checkpoint.pth.tar'
        elif modality=='Flow':
            pretrained_file='./weightFile/p3d_flow_199.checkpoint.pth.tar'
        weights=torch.load(pretrained_file)['state_dict']
        model.load_state_dict(weights)
    return model

def transfer_model(model, model_type='P3D', num_classes=2):
...
def get_1x_lr_params(model):
    ...
    """
    b = [model.conv4a, model.conv4b, model.conv5a, model.conv5b, model.fc6, model.fc7]
    # b = [model.conv5a, model.conv5b, model.fc6, model.fc7]
...
if __name__ == '__main__':
    ...
    #path = '/home/proj_vode/P3D_data/assault/'
    #path = '/home/proj_vode/P3D_data/burglary/'
    path = '/home/proj_vode/P3D_data/kidnap/'
    ...
    for i in file_list:
        ...
        trans = transforms.Compose([transforms.CenterCrop(160),
                                transforms.ToTensor()])
        ...
        make = torch.zeros((1,3,16,160,160))

        model = P3D199(pretrained=True, num_classes=400)
        model = transfer_model(model=model, model_type='P3D', num_classes=2)
        model = model.cuda()
        ...
        for j,data in enumerate(trainloader):
            ...
            #target_dir = '/home/proj_vode/data_numpy_P3D_crop/assault/'+i+'/'
            #target_dir = '/home/proj_vode/data_numpy_P3D_crop/burglary/'+i+'/'
            target_dir = '/home/proj_vode/data_numpy_P3D_crop/kidnap/'+i+'/'

            if not os.path.exists(target_dir):
                os.makedirs(target_dir)

            np.save(target_dir+trainset.classes[labels[0]],result) 

            #np.save('/home/dahee333/P3D/numpy/'+trainset.classes[labels[0]],result)
            #print(result)
            #print(result.shape)
'''