...
if __name__ == "__main__":
    ...
    jpg_data = VideoDataset(jpg_path = '/projects/vode/team1/16_segment', video_list_txt = 'test_trespass.txt', class_name = class_name, num_channel = 3, clip_len=16)
    trainloader = DataLoader(jpg_data, batch_size=1, shuffle=False, num_workers=4)

    x3d_version = 'M'
    model = generate_model(x3d_version, base_bn_splits=1)

    weights=torch.load('./weightFile/x3d_multigrid_kinetics_fb_pretrained.pt')["model_state_dict"]
    ...
    for j,data in enumerate(trainloader):
        images, video_segment_path, videoName_to_segName  = data
        print(videoName_to_segName)    

        out = model(images)
        out = out.cpu()
        result = out.detach().numpy()

        target_dir = '/projects/vode/test_numpy_sec/X3D_preproc/' + class_name + '/' + videoName_to_segName[0].split('/')[0] + '/'
        
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)

        np.save(target_dir+videoName_to_segName[0].split('/')[1],result)